...
import { Terminal, Settings, User, FileDigit, Binary, Heart, LineChart, Cpu, Activity, SquareTerminal } from 'lucide-vue-next'
  ...
  <div class="flex h-screen w-screen bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-200 font-sans transition-colors duration-200">
    ...
    >
      ...
      <router-link 
        to="/shell" 
        class="group flex flex-col items-center justify-center gap-1.5 p-2.5 rounded-xl transition-all duration-200 w-[88px]"
        :class="route.path === '/shell' ? 'bg-blue-600 text-white shadow-md shadow-blue-900/50 scale-105' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200 active:scale-95'"
        :title="t('nav.shell')"
        :aria-label="t('nav.shell')"
        :aria-current="route.path === '/shell' ? 'page' : undefined"
      >
        <SquareTerminal class="w-6 h-6 shrink-0 transition-transform group-hover:scale-110" aria-hidden="true" />
        <span class="text-[10px] font-medium tracking-wide w-full text-center whitespace-nowrap px-1">{{ t('nav.shell') }}</span>
      </router-link>
